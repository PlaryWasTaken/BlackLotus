...
export default new Command({
  ...
  async run({ client, interaction }) {
    ...
    if (isSyndicate) {
      await client.syndicateManager
        .delete(id)
        .then(async () => {
          await interaction.reply({
            ephemeral: true,
            content: "Servidor removido com sucesso",
          });
        })
        .catch(async (err) => {
          console.log(err);
          await interaction.reply({
            ephemeral: false,
            content: `Não achei esse servidor ou ocorreu um erro. Esquisito...\nDe qualquer modo aqui está o erro:\n\`\`\`${err}\`\`\``,
          });
        });
    } else {
      await client.blackLotusManager
        .delete(id)
        .then(async () => {
          await interaction.reply({
            ephemeral: true,
