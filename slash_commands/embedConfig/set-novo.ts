...
export default new Command({
  ...
  async run({ client, interaction }) {
    if (interaction.user.id === "1021932527037980673")
    ...
    switch (interaction.options.getSubcommand()) {
      ...
      case "cor":
        ...
        await interaction.reply({
          ephemeral: true,
          content: `A cor do embed foi setada para ${color}`,
        });
      ...
      case "titulo":
        ...
        await interaction.reply({
          ephemeral: true,
          content: `O título do embed foi setado para ${title}`,
        });
        client.mainEmbed.message.save();
        break;
      case "descricao":
      ...
      case "imagem":
        ...
        await interaction.reply({
          ephemeral: true,
          content: `A imagem do embed foi setada para ${image}`,
        });
        client.mainEmbed.message.save();
        break;
      case "prefixo":
      ...
      case "staff":
        ...
        if (data3.modules.blackLotus.staffs.find((e) => e.id === user2.id))
          return interaction.reply({
      ...
      case "aparecerembed":
        ...
        await interaction.reply({
          ephemeral: true,
          content: `O servidor ${data.modules.blackLotus.displayName} agora ${
            interaction.options.getBoolean("aparecer") ? "aparece" : "não aparece"
          } no embed`,
        });
        break;
    }
  ...
  async autocomplete({ interaction }) {
    const servers = await serverModel
      .find({
        $or: [
          { "blackLotus.displayName": new RegExp(interaction.options.getString("servidor"), "i") },
          { id: interaction.options.getString("servidor") },
        ],
      })
      .limit(25);
    const options = [];
