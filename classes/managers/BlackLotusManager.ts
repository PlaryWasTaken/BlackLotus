...
export default class BlackLotusManager {
  ...
  fetch(id: string): Promise<BlackLotusGuild> {
    return new Promise(async (resolve, err) => {
      ...
      const guildData = await GuildModel.findOne({
        ...
        $or: [{ blacklisted: false }, { blacklisted: { $exists: false } }],
      });
  ...
  create({
    ...
    return new Promise(async (resolve, err) => {
      ...
      const ConstellationHandle = new Constellation(
        id,
        this.client,
        guild,
        undefined
      ...
      await profile.save();
      const guildHandle = new BlackLotusGuild(
        id,
        this.client,
        guild,
        profile as any
  ...
  delete(
    id: string,
    reason: string = "Guilda foi deletada, não estava com o bot (Razão padrão)"
  ): Promise<this> {
    return new Promise(async (resolve, err) => {
      const guildData = await GuildModel.findOne({
        id: id,
      ...
      if (!guildObj) {
        this.client.logger.info(
          `Guild ${id} was not found when trying to delete it, this means the guild has removed the bot`
        );
        const role = await this.client.blackLotus.roles
          .fetch(guildData.modules.blackLotus.role)
          .catch(() => {});
      ...
      const guild = new BlackLotusGuild(
        id,
        this.client,
        guildObj,
        guildData as any
  ...
  fetchByKV(filter: Object): Promise<Array<BlackLotusGuild>> {
    return new Promise(async (resolve) => {
      ...
      if (!filter["blacklisted"] && !filter["$or"])
        filter["$or"] = [
          { blacklisted: false },
          { blacklisted: { $exists: false } },
        ]; // This is to make sure we only get guilds that are not blacklisted, unless the filter specifies otherwise
      const guildData = await GuildModel.find(filter);
      if (!guildData || guildData.length === 0) return [];
      const guilds = [];
      for (const guild of guildData) {
        const guildObj = await this.client.guilds
          .fetch(guild.id)
          .catch(() => {});
        if (!guildObj) {
          this.client.logger.warning(
            `Guild ${guild.id} was not found when trying to fetch it`
          );
          continue;
        }
        guilds.push(
          new BlackLotusGuild(guild.id, this.client, guildObj, guild as any)
        );
      }
      resolve(guilds);
    });
