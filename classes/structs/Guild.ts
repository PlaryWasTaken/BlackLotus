...
import {ExtendedClient, GuildDocument} from "../../types";
import {Guild as DGuild } from "discord.js";
async function removeRoleFromStaffs(Class: Guild, client: ExtendedClient) {
    for (const staff of Class.staffs) {
        ...
        for (const server of otherServers) {
            const guildData = await client.guildManager.fetch(server.id).catch(() => {})
            ...
            if (!staff2.role) {
                ...
                guildData.data.blackLotus.staffs.splice(index, 1)
...
export default class Guild {
    ...
    get displayedName() {
        return this.data.blackLotus.displayName
    }
    get representant() {
        return this.data.blackLotus.representant
    }

    updateName(name: string): Promise<this> {
        return new Promise(async (resolve, err) => {
            this.data.blackLotus.displayName = name
            this.data.blackLotus.embedWorthy = name.length <= 25
