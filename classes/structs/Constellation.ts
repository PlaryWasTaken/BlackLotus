...
export default class ConstellationHandler {
  ...
  fetch(): Promise<Constellation> {
    return new Promise(async (resolve, err) => {
      const memberAmount = await this.guild.members
        .fetch()
        .then((collection) => {
  ...
  updateConstellation(): Promise<Constellation> {
    return new Promise(async (resolve, err) => {
      this.client.logger.info(
        `Updating Constellation for ${this.data.modules.blackLotus.displayName}`
      );
      const constellation = await this.fetch();
      if (!this.data.modules.blackLotus.role) return err("No role");
      const role = await this.client.blackLotus.roles
        .fetch(this.data.modules.blackLotus.role)
        .catch(() => {});
      const constellationRole = await this.client.blackLotus.roles
        .fetch(constellation.roleId)
        .catch(() => {});
      const representant = await this.client.blackLotus.members
        .fetch(this.data.modules.blackLotus.representant)
        .catch(() => {});
      if (!role || !constellationRole || !representant)
      ...
      if (!this.data.modules.blackLotus.staffs)
        this.data.modules.blackLotus.staffs = [];
      this.data.modules.blackLotus.staffs.push(
        this.data.modules.blackLotus.representant
      ); // Everything applies the same to the representant so just add him in here instead of doing something else
      for (const staff of this.data.modules.blackLotus.staffs) {
        if (!staff.id) continue;
        const member = await this.client.blackLotus.members
          .fetch(staff.id)
          .catch(() => {});
        if (!member) {
          // Just in case the member left the server while the bot was offline
          this.data.modules.blackLotus.staffs =
            this.data.modules.blackLotus.staffs.filter(
              (staff2) => staff2.id !== staff.id
            );
        ...
        await member.roles.remove(
          this.data.modules.blackLotus.constellation.roleId
        );
        const staffGuilds = await this.client.blackLotusManager.fetchByKV({
          "modules.blackLotus.staffs": { $elemMatch: { id: staff.id } },
        });
      ...
      this.data.modules.blackLotus.staffs =
        this.data.modules.blackLotus.staffs.filter(
          (staff) => staff.id !== this.data.modules.blackLotus.representant
        );
      await GuildModel.findOneAndUpdate(
        { id: this.data.id },
        { $set: { "modules.blackLotus.constellation": constellation._id } }
      );
