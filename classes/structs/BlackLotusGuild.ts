...
async function removeRoleFromStaffs(
  Class: BlackLotusGuild,
  client: ExtendedClient
) {
  for (const staff of Class.staffs) {
    const member = await client.blackLotus.members
      .fetch(staff.id)
      .catch(() => {});
    if (!member) continue;
    Class.roles.push(staff.role);
    let otherServers = await serverModel.find({
      "blackLotus.staffs": { $elemMatch: { id: staff.id } },
    });
    ...
    for (const server of otherServers) {
      const guildData = await client.blackLotusManager
        .fetch(server.id)
        .catch(() => {});
      ...
      if (!staff2.role) {
        ...
        const index = guildData.staffs.findIndex(
...
export default class BlackLotusGuild {
  ...
  constructor(
    ...
    this.data = data;
    if (this.guild) {
      this.constellation = new ConstellationHandler(
        id,
        client,
        this.guild,
        data
      ...
      client.logger.warning(
        "No guild data provided for guild " +
          data.modules.blackLotus.displayName +
          " (" +
          id +
          ")"
      );
    ...
    this.roles.push(
      this.data.modules.blackLotus.role,
  ...
  delete(
    reason: string = "Guilda foi deletada e tinha o bot no servidor (Razão padrão)"
  ): Promise<this> {
    return new Promise(async (resolve, err) => {
      if (this.data.blacklisted)
        return err("Guild is blacklisted and cannot be deleted");
      this.staffs.push({
      ...
      await this.guild.invites
        .delete(this.data.modules.blackLotus.invite)
        .catch(() => {}); // Clean up the invite the bot created
