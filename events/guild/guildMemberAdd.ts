    ...
    if (constelation && constelation.name !== guildData.data.blackLotus.constelation.name) {
        ...
        if (!guildData.data.blackLotus.representant) {
            const embed = new Discord.EmbedBuilder()
                ...
                .setDescription(`O servidor ${guild.name} (Id: ${guild.id}) não tem representante e deve mudar de constelacão (Indo de: ${guildData.data.blackLotus.constelation.name} para ${constelation.name}), por favor, adicione um representante para o servidor${await blackLotus.members.fetch(guild.ownerId).catch(() => {
                    return undefined
