...
export default new Event().setData("button.acceptServer", async (client, interaction, args) => {
    ...
    await interaction.reply({content: 'Adicionando...', ephemeral: true})
    const blackLotus = client.guilds.cache.get('896047806454837278')
    const member = await blackLotus.members.fetch(args[2]).catch(() => {
    })
    if (!member) return interaction.followUp({ephemeral: true, content: `O membro representante não está no servidor!`})
    const server = await client.guildManager.create({ // Try to create a new server in the database, fails if the server is already in the database
        id: guild.id,
        repId: args[2],
        invite: inviteCode
    }).catch(() => {
    ...
    server.data.blackLotus.role = newRole.id
